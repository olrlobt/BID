...
export default function BidPage() {
  ...
  useEffect(() => {
    if (isInitialRender) {
      ...
      setNotIncluded(dummyNotIncluded);
      setIncluded(dummyIncluded);
  ...
  //   const newCoupon = {no: 10, name: '새 쿠폰', description: '오우 신상이다 신상!', startPrice: 340};
  //   setNotIncluded(notIncluded => [...notIncluded, newCoupon]);
  // }
        ...
        {isTeacher ? (
          <div className={styled.couponListWrapper}>
            <div>
              ...
              <CouponList coupons={notIncluded} removeCoupon={removeCoupon} />
            </div>
            <div>
              <div className={styled.couponListTitle}>등록된 쿠폰</div>
              <CouponList coupons={included} removeCoupon={removeCoupon} />
            </div>
          </div>
        ) : (
